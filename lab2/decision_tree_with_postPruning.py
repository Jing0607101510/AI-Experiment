...
def build_cart_tree(data_set, attrs, attrs_value):
	...
	if len(set(class_labels)) == 1:
		...
		next_attr = cart_find_next_attr(data_set)#next_attr是索引
		attr_name = attrs.pop(next_attr)
		node = {attr_name:{}}
		for type in attrs_value[attr_name]:##################
			node[attr_name][type] = mode
...
#后剪枝
def postCutTree(tree, data_set, test_set, attrs):
	'''
	:param tree : 决策树
	:param data_set : 数据集
	:param test_set : 验证集
	...
	；return 子树或者叶子节点
	'''
	class_labels = []#获取所有的数据的样本值
	for data in data_set:
		class_labels.append(data[-1])
	mode = get_mode(class_labels)#获取出现最多的样本值

	root = list(tree.keys())[0]
	Dict = tree[root]#字典代表的子树
	index = attrs.index(root)
	attrs.remove(root)#说明已经使用了这个特征

	#递归验证
	for key in Dict.keys():
		if type(Dict[key]).__name__ == 'dict':
			tree[root][key] = postCutTree(Dict[key], get_new_data_set(data_set, index, key), get_new_val_set(test_set, root, key), attrs[:])
	#判断是否剪枝对正确率的影响
	if error_post_cut(tree, test_set) <= error_not_post_cut(mode, test_set):
		return tree#不剪枝的正确率高
	else:
		return mode#剪枝正确率高



def error_post_cut(tree, test_set):
	return len(test_set) - test_tree(tree, test_set)


def error_not_post_cut(mode, test_set):
	error = 0
...
def k_fold_crossValidation(data_set, attrs, choose):
	...
	for i in range(k):
		...
		if choose == 1:
			...
			id3_tree = postCutTree(id3_tree, train_set[:], validation_set[:], attrs[:])
			num = test_tree(id3_tree, validation_set)
			accurary +=  num / len(validation_set)
			predict_test_set(id3_tree)

		elif choose == 2:
			c45_tree =  build_c45_tree(train_set[:], attrs[:], attrs_value)
			c45_tree = postCutTree(c45_tree, train_set[:], validation_set[:], attrs[:])
			num = test_tree(c45_tree, validation_set)
			accurary += num / len(validation_set)
			predict_test_set(c45_tree)

		elif choose == 3:
			cart_tree =  build_cart_tree(train_set, attrs[:], attrs_value)
			cart_tree = postCutTree(cart_tree, train_set[:], validation_set[:], attrs[:])
			num = test_tree(cart_tree, validation_set)
			accurary += num / len(validation_set)
			predict_test_set(cart_tree)
		print(num, "/", len(validation_set))
