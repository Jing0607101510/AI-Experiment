...
def build_id3_tree(data_set, attrs, attrs_value, test_set):
	...
	if len(attrs) == 0:
		...
		unique_fea = [data_set[i][next_attr] for i in range(len(data_set))]
		for type in set(unique_fea):
			node[attr_name][type] = get_mode([labels[-1] for labels in get_new_data_set(data_set, next_attr, type)])
		
		if error_pre_cut(test_set, mode) >= error_not_pre_cut(test_set, node):
			for type in set(unique_fea):
...
def error_pre_cut(test_set, mode):
	sum = 0
...
def error_not_pre_cut(test_set, tree):
