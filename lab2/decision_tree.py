...
def predict_test_set(tree):
	result = []
	acc = 0
	with open("car_test_with_label.csv", "r") as file:
		reader = csv.reader(file)
		#test_set = [data[:-1] for data in reader]
		test_set = [data for data in reader]
		for each_line in test_set:
			predict_result = predict(tree, each_line[:-1])
			result.append(predict_result)
			if predict_result == each_line[-1]:
				acc += 1
	print("test_set_accuarry:", acc / len(test_set))
